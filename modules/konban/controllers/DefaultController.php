...
class DefaultController extends Controller
{
    ...
    public function actionGetuserbyitemid(){
        $myItem = $_REQUEST["itemID"];
        
        $ownerID = (new \yii\db\Query())->select([
            'ownerID'
        ])
        ->from('items')
        ->where([
        ...
        $myusername = (new \yii\db\Query())->select([
            'username'
        ])
        ->from('user')
        ->where([
    ...
    public function actionRemoveproject()
    {
        ...
        Yii::$app->db->createCommand()
            ->delete('projects', 'projectID =' . $myPID)
            ->execute();
        ...
        $mytodo = (new \yii\db\Query())->select([
            '*'
        ])
            ->from('itemStatus')
            ->where([
...
<div class="panel panel-default">
    ...
    public function actionRemoveitem()
    {
        ...
        Yii::$app->db->createCommand()
            ->delete('items', 'itemID =' . $myItem)
            ->execute();
        Yii::$app->db->createCommand()
            ->delete('itemStatus', 'itemID =' . $myItem)
            ->execute();
        Yii::$app->db->createCommand()
            ->delete('messages', 'itemID =' . $myItem)
            ->execute();
        echo "item removed";
    }
    ...
    public function actionGetitemdetails()
    {
        ...
        $myItem = (new \yii\db\Query())->select([
            '*'
        ])
            ->from('items')
            ->where([
        ...
        $myStatus = (new \yii\db\Query())->select([
            '*'
        ])
            ->from('itemStatus')
            ->where([
